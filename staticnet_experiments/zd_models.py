...
from neuro_data.static_images.zd_neurodata import StaticMultiDataset as zd_StaticMultiDataset, StaticMultiDataset2 as zd_StaticMultiDataset2, StaticScan as zd_StaticScan
from neuro_data.static_images.data_schemas import StaticMultiDataset, StaticScan
...
schema = dj.schema('zhiwei_neurostatic_models')
...
class Model(dj.Computed):
    ...
    @property
    def key_source(self):
        return super().key_source - BadConfig

    class TestScores(dj.Part):
        definition = """
        -> master
        -> zd_StaticMultiDataset.Member
        ---
    ...
    def evaluate(self, key=None):
        ...
        for readout_key, testloader in testloaders.items():
            ...
            member_key = (zd_StaticMultiDataset.Member() & key & dict(name=readout_key)).fetch1(dj.key)
...
@schema
@gitlog
class Model2(dj.Computed):
    definition = """
    -> zd_StaticMultiDataset2
    -> NetworkConfig
    -> Seed
    ---
    val_corr                  : float              # validation correlation (single trial)
    ...
    def evaluate(self, key=None):
        ...
        for readout_key, testloader in testloaders.items():
            ...
            member_key = (zd_StaticMultiDataset2.Member() & key & dict(name=readout_key)).fetch1(dj.key)
            member_key.update(key)
...
class TrainedModel(dj.Computed):
    ...
    val_corr                  : float              # validation correlation (single trial)
    model                     : external-static    # stored model
    lr_shift_epochs           : longblob           # list of epochs when the learning rate changes 
    lr_shift_batchs           : longblob           # list of batch numbers when the learning rate changes
    all_train_losses          : longblob           # training loss over iterations 
    all_train_corrs           : longblob           # training correlation over iterations 
    all_val_losses            : longblob           # validation loss over iterations 
    all_val_corrs             : longblob           # validation correlation over iterations
    """
...
@schema
@gitlog
class TempModel(dj.Computed): 
    definition = """   # temporary model trained for benchmark
    -> StaticMultiDataset
    -> NetworkConfig
    -> Seed
    ---
    val_corr                  : float              # validation correlation (single trial)
    model                     : external-static    # stored model
    ip_address                : varchar(32)        # IP address of the machine where the entry was populated
    """
    ...
    def make(self, key):
        ...
        # --- set seed
#         seed = (Seed() & key).fetch1('seed')
        seed = 1009 # manually set seed to 1009 to overwrite the original seed
        log.info('Setting seed to {}'.format(seed))
        set_seed(seed)
        try:
            updated_key, scores, unit_scores = NetworkConfig().train(key)
