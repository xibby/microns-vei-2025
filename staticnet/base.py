...
class BehaviorNet(nn.Module):
    # this model is not designed for multiple dataset training!
    def __init__(self, core, readout, modulator=None, nonlinearity=None, shifter=None):
        super().__init__()
    ...
    @staticmethod
    def attach_behavior(x, behavior):
        beh_channel = behavior[...,None,None] * torch.ones(x.shape, device=behavior.device)
        return torch.cat([x, beh_channel], dim=1)
    
    def forward(self, x, readout_key=None, behavior=None, eye_pos=None):
        if behavior is not None:
            x = self.attach_behavior(x, behavior)
        if readout_key is None:
            readout_key = self.readout_key
...
class LocomotionNet(nn.Module):
    ...
    @staticmethod
    def attach_behavior(x, behavior):
        beh_channel = behavior[:,2:,None,None] * torch.ones(x.shape, device=behavior.device)  # only attach the last dimension of behavior information (locomotion), one element slice to keep dimension
        return torch.cat([x, beh_channel], dim=1)
