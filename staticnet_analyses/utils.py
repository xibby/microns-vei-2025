...
def discretized_forward(self, x, shift=None):
    ...
    for xx in self.gauss_pyramid(x):
        ...
        pools.append(F.grid_sample(xx, adj_grid, align_corners=True))
...
def make_step(net, src, step_size=1.5, sigma=None, precond=0, step_gain=1,
    ...
    Arguments:
        ...
        train_norm (float): Match standard deviation of the image fed to the network to
            this value; however, norm of the optimized image is not constrained. Expressed
            in pixel values (0-255). Unused if None.
        norm (float): Match standard deviation of the image to this value after each
            update. Expressed in pixel values (0-255). Unused if None.
...
def create_gabor(height, width, orientation, phase, wavelength, sigma=0.1, dx=0, dy=0, only_grating=False):
