...
def best_model(m, aggr=None):
    ...
    target = m * configs.NetworkConfig.CorePlusReadout()
...
def best_model_new(m, aggr=None):
    ...
    target = m * corrs_per_model * configs.NetworkConfig.CorePlusReadout()
    aggr_target = StaticMultiDataset() * DataConfig()
    if aggr is not None:
        aggr_target = aggr_target * aggr
...
def prepare_data(key, readout_key):
    ...
    mu = trainset.statistics['images'][trainset.stats_source]['mean'][()].item()
    s = trainset.statistics['images'][trainset.stats_source]['std'][()].item()
...
#     key_source = experiment.Scan & StaticMultiDataset.Member
    ...
    def make(self, key):
        ...
        closest_calibration = (delta * min_diff & 'delta_time LIKE min_diff').fetch()[0]
...
class TargetModel(dj.Computed):
    ...
    def fill(self, group_id, dryrun=False):
        ...
        core_types = [configs.CoreConfig.Stacked2d, configs.CoreConfig.Linear]
        # check if downsample should be made 0
        ro_types = [configs.ReadoutConfig.ModifiedSpatialTransformerPyramid2d & 'downsample=1',
                    configs.ReadoutConfig.ModifiedSpatialTransformerPyramid2d & 'downsample=0',
                    configs.ReadoutConfig.SpatialTransformerPooled2d,
                    configs.ReadoutConfig.SpatialTransformerPyramid2d & 'downsample=1',
                    configs.ReadoutConfig.SpatialTransformerPyramid2d & 'downsample=0']
        shifter_types = [configs.ShifterConfig.MLP, configs.ShifterConfig.StaticAffine2d]
        mod_types = [configs.ModulatorConfig.MLP]
        ...
        targets = []
        for cond in conds:
            keys = best_model(Model & (configs.NetworkConfig.CorePlusReadout & cond) & restr).fetch('KEY')
            if len(keys):
...
class OracleRankedUnit(dj.Computed):
    ...
    def make(self, key):
        keys, corr = (TargetDataset.Unit * stats.Oracle.UnitScores & key).fetch('KEY', 'pearson', order_by='pearson DESC')
        N = len(keys)
        unit_keys = []
...
class ModelGroup(dj.Computed):
    ...
    @property
    def key_source(self):
        return TargetDataset & (configs.NetworkConfig.CorePlusReadout * TargetModel)

    def make(self, key):
        best_cnn = best_model(Model & key & (configs.NetworkConfig.CorePlusReadout * TargetModel & key & [configs.CoreConfig.GaussianLaplace]))
        best_lin = best_model(Model & key & (configs.NetworkConfig.CorePlusReadout * TargetModel & key & [configs.CoreConfig.StackedLinearGaussianLaplace]))
...
#TODO: Delete HighUnitSelection, has nothing and it's wrong anyway (I think)
@schema
...
def adjust_img_stats(img, mu, sigma, img_min=0, img_max=255, mask=None, max_gain=6000, min_gain=0.001,
    ...
    """ Match contrast (standard deviation) of the image (inside the mask) to sigma."""
    if mask is None:
        mask = np.ones_like(img)

    # Compute residual (mask - mean)
    masked_mean = (img * mask).sum() / mask.sum()
    residuals = img - masked_mean

    # Helper function to mask a residuals image
    def mask_image(im, offset=0):
        """ Mask im, move background to mu + offset and clip at valid image range."""
        masked_im = (im * mask) + mu
        return np.clip(masked_im + offset, img_min, img_max)

    # Find best gain and offset
    masked_image = mask_image(residuals)
    if masked_image.std() < 1e-8:  # flat image
        warnings.warn('Zero standard deviation detected.')
        img = mask_image(residuals)
        unmasked_img = np.clip(residuals + mu, img_min, img_max)
    else:
        # Define minimum and maximum possible gain
        min_gain = min_gain
        max_gain = min(max(img_max - mu, mu - img_min) / np.min(np.abs(residuals)[mask > 0]), max_gain)

        # Define offsets to try for each gain
        possible_offsets = np.linspace(-30, 30, 1000)[:, None, None]

        # Define initial gain to try (best possible gain)
        best_gain = sigma / masked_image.std()

        # Iterate over diff gains looking for the offset that will get the image closer to the desired mean
        gain = best_gain
        masked_imgs = []
        unmasked_imgs = []
        contrasts = []
        while True:
            # Find best offset for current gain
            if verbose:
                print('Trying gain', gain)
            masked_images = mask_image(residuals * gain, possible_offsets)  # num_offsets x h x w
            best_offset_idx = np.argmin(np.abs(masked_images.mean(axis=(-1, -2)) - mu))

            # Save images
            masked_img = masked_images[best_offset_idx]
            masked_imgs.append(masked_img)
            unmasked_img = np.clip(residuals * gain + mu +
                                   possible_offsets[best_offset_idx], img_min, img_max)
            unmasked_imgs.append(unmasked_img)

            # Save current contrast
            contrast = masked_img.std()
            if verbose:
                print('Got mean and contrast', masked_img.mean(), contrast)
            contrasts.append(contrast)

            # Update gain for next iteration
            if best_gain >= 1: # when desired std was bigger than masked_std
                gain *= base_ratio  # increase gain a bit
                if contrast > sigma:  # stop if masked_std/contrast is already bigger than sigma
                    break
            else:
                gain /= base_ratio
                if contrast < sigma:
            ...
            # gain went out of bounds
            if gain < min_gain or gain > max_gain:
                break

        # Finally, select the gain that has the closest sigma to the desired sigma
        best_sigma_idx = np.argmin(np.abs(np.array(contrasts) - sigma))
        masked_img = masked_imgs[best_sigma_idx]
        unmasked_img = unmasked_imgs[best_sigma_idx]
    if verbose:
...
class MultiModelEvaluation(dj.Computed):
    ...
    key_source = TargetModel * TargetDataset & configs.NetworkConfig.CorePlusReadout
    ...
    def make(self, key):
        ...
        scores, unit_scores = [], []
        oracle_units = stats.Oracle.UnitScores * TargetDataset.Unit & key
...
class MEI(dj.Computed):
    ...
    def key_source(self):
        ...
        return TargetModel() * MEIParameter() * TargetDataset.Unit & configs.NetworkConfig.CorePlusReadout
...
class BestTightMaskedImageNet(dj.Computed):
    ...
    def make(self, key):
        ...
        key['image_id'] = image_ids[pos]
        key['img_activation'] = img_activations[pos]
        key['unmasked_activation'] = evaluate_image(images[pos]) # * this evaluates in the
        # original unmasked image, not on the one with center matched in contrast with masked
...
class GradRF(dj.Computed):
    ...
    key_source =  TargetModel() * MEIParameter() * TargetDataset.Unit & configs.NetworkConfig.CorePlusReadout
...
class SpotCenteredMEI(dj.Computed):
    ...
    def key_source(self):
        # restriction by CorePlusReadout is needed to link dataconfig with the appropriate model
        return TargetModel() * MEIParameter() * ImageConfig() * TargetDataset.Unit & configs.NetworkConfig.CorePlusReadout
...
class BestGabor(dj.Computed):
    ...
    def key_source(self):
        # restriction by CorePlusReadout is needed to link dataconfig with the appropriate model
        return TargetModel * ProcessedImageConfig * GaborRange * TargetDataset & configs.NetworkConfig.CorePlusReadout
...
class OptimalGabor(dj.Computed):
    ...
    def key_source(self):
        ...
        return (TargetModel * ProcessedImageConfig.proj() * GaborLimits.proj() *
                TargetDataset.Unit & configs.NetworkConfig.CorePlusReadout)
