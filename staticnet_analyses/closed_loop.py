...
# import os
# os.chdir("/src/staticnet_mei/")
# from static_analyses import base as base_jk
# from neurodata_prep import model_data
...
class StimType(dj.Lookup):
    definition = """
    stim_type: varchar(64)  # name of the stim type
    ---
    ...
    contents = [
        ...
        ('meidiffblur', 'Closed loop comparison of MEIs generated at different gradient smoothing levels'),
        ('meidiffbluract', 'Closed loop comparison of MEIs generated at different gradient smoothing levels selected by model activation'),
        ('lei', 'Closed loop comparison of MEI vs LEI'),
        ('maskunmasknat', 'Closed loop comparison of masked vs unmasked natural images'),
        ('maskunmaskmei', 'Closed loop comparison of masked MEI vs unmasked MEI'),
        ('meiversions', 'Closed loop comparing MEIs generated with deepdraw versus featurevis MEIs'),
        ('divmeiV1pixel', 'comparison of original MEI and pixel diverse MEIs for V1 neurons'),
        ('divmeiV1vae', 'comparison of original MEI and VAE diverse MEIs for V1 neurons'),
        ('divmeiHVpixel', 'comparison of original MEI and pixel diverse MEIs for higher visual neurons'),
        ('divmeiV1pixelv2', 'version2 comparison of original MEI and pixel diverse MEIs for V1 neurons'),
        ('divmeiV1vaev2', 'version2 comparison of original MEI and VAE diverse MEIs for V1 neurons'),
        ('blurdei_pixel', 'comparison of original MEI and pixel DEIs with blur_sigma=1 (mei_params = 5)'), 
        ('blurdei_pixel2', 'comparison of original MEI and pixel DEIs with blur_sigma=1 (mei_params = 3)'),
        ('pixeldei_loop9', 'comparison of original MEI and pixel DEIs with diverse_params 1 and loop params 9'),
        ('pixeldei_loop10', 'comparison of original MEI and pixel DEIs with diverse_params 26 and loop params 10'),
        ('searched_nat', '5000 imagenet images searched for active learning for target neurons'),
        ('actlearn_dei', 'comparison and MEI and DEIs for neurons targeted for active learning'),
        ('dei_and_searched', "2500 DEIs and 2500 searched natural images for active learning"),
        ('masked_imagenet', "5000 masked natural images"), 
        ('mei_collection', "5000 random V1 MEIs from previous scans"),
        ('dei_collection', "25 DEIs for each of the top ranked 200 neurons for active learning"),
        ('px_lum_10dei', 'comparison of MEI and 10 DEIs per neuron optimized in luminance and pixel space'),
        ('pxdei_params37', 'comparison of original MEI and pixel DEIs with diverse_params 37'),
        ('half_single_half_repeats', '2700 natural images repeated once and 300 natural images repeated 10 times'),
        ('optim_imagenet', 'selected collection of imagenet images aiming to achieve optimal model performance'),
        ('half_optim_half_random', 'half collection of optiml_imagenet and half collection of random imagenet'),
        ('half_mei_half_imagenet', 'half mei_collection and half imagenet')]
...
class ProximityCellMatch(dj.Computed):
    ...
    def make(self, key):
        ...
        # #             StaticMultiDataset.Member & src_key) # TODO: This only works if MEIs were shown during stimulus
...
@schema
class ProximityCellMatchReso(dj.Computed):
    definition = """
    -> reso.StackCoordinates.proj(src_session='session', src_scan_idx='scan_idx', src_field='field')
    -> reso.ScanInfo
    ---
    """
    ...
    class UnitMatch(dj.Part):
        definition = """
        -> master
        -> reso.StackCoordinates.UnitInfo.proj(src_session='session', src_scan_idx='scan_idx', src_field='field', src_unit_id='unit_id')
        -> reso.ScanInfo
        ---
        -> reso.ScanSet.Unit # matched unit id
        match_distance: float   # distance between matched units in um
        """
    ...
    def key_source(self):
        valid_combo = (ClosedLoopScan() & 'mei_source=1 and loop_group > 0').proj(src_session='session', src_scan_idx='scan_idx',
                                                               src_loop_group='loop_group') * \
                      (ClosedLoopScan() & 'mei_source != 1').proj('loop_group') & 'src_loop_group = loop_group'
        restr = reso.StackCoordinates.proj(**src_map) * reso.StackCoordinates() & valid_combo
        return reso.StackCoordinates.proj(**src_map) * reso.ScanInfo & restr
    ...
    def make(self, key):
        ...
        dest_unit_keys, dest_x, dest_y, dest_z = (reso.StackCoordinates.UnitInfo() & key & soma_units).fetch('KEY',
                                                                                                             'stack_x',
                                                                                                             'stack_y',
                                                                                                             'stack_z',
                                                                                                             order_by='unit_id')
...
@schema
class BestProximityCellMatchReso(dj.Computed):
    definition = """
    -> reso.ScanSet.Unit.proj(src_session='session', src_scan_idx='scan_idx', src_unit_id='unit_id')
    -> reso.ScanInfo
    ---
    -> reso.ScanSet.Unit   # most frequently matched unit id
    match_freq:  int       # how many times it was matched
    total_stacks: int      # number of stacks used to match
    mean_distance: float   # average match distance (in um)
    """
    ...
    @property
    def key_source(self):
        src_scan = reso.ScanInfo.proj(src_session='session', src_scan_idx='scan_idx')
        dst_scan = reso.ScanInfo
        return src_scan * dst_scan * shared.SegmentationMethod & ProximityCellMatchReso

    def make(self, key):
        df = pd.DataFrame((ProximityCellMatchReso.UnitMatch() & key).fetch())
        attrs = list(key) + ['src_unit_id']
...
@schema
class AllCellMatchOracleCorrelation(dj.Computed):
    definition = """   # computes oracle correlation of all matched cells between mei source scan and mei scans
    -> ClosedLoopScan 
    ---
    units: longblob  # unit ids from mei source scan
    corr_matrix: longblob  # correlation matrix - rows are source and columns are targets (matched)
    """
    @property
    def key_source(self):
        return ClosedLoopScan & BestProximityCellMatch & 'mei_source = False'

    def make(self, key):
        src_key = (ClosedLoopScan & (dj.U('loop_group') & (ClosedLoopScan() & key)) & 'mei_source = True').fetch1('KEY')
        # Find all units matching the source units
        src_units, target_units = (BestProximityCellMatch & key & {'src_session': src_key['session'], 'src_scan_idx': src_key['scan_idx']}).fetch('src_unit_id', 'unit_id', order_by='src_unit_id')
        unit_map = OrderedDict(zip(src_units, target_units))
        ...
        units = combined.reset_index()['unit_id']
        key['units'] = units.values[:len(src_units)]
